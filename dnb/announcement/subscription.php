...
function saveSubscription($pdo, $data)
{
    ...
    $announcement_types = $data['announcement_types'] ?? []; // Updated to handle announcement type IDs

    if (empty($usn) || empty($dept_id) || empty($announcement_types)) {
    ...
    try {
        ...
        $insertQuery = "INSERT INTO subscription(usn, dept_id, announcement_type_id) VALUES (:usn, :dept_id, :announcement_type_id)";
        $stmt = $pdo->prepare($insertQuery);

        foreach ($announcement_types as $announcement_type_id) {
            $stmt->execute([
                'usn' => $usn,
                'dept_id' => $dept_id,
